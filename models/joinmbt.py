...
class JoinMbt(JointAutoregressiveHierarchicalPriors):
    ...
    def compress(self, x, stream_name='joo'):
        ...
        fileobj.write(
            np.array([len(z_strings[0]), minmax], dtype=np.uint16).tobytes())
        ...
        for h in tqdm(range(H)):
            for w in range(W):
                y_crop = y_hat[0: 1, :, h: h + kernel_size,
                               w: w + kernel_size]     # (1, C, 5, 5)
                ctx_p = F.conv2d(
                    y_crop,
                ...
                scales_crop, means_crop = gaussian_params.chunk(
                    2, 1)   # (1, C, 1, 1)
                ...
                for i in range(len(non_zero_idx)):
                    ...
                    scale = self.gaussian_conditional.lower_bound_scale(
                        scale_p)
                    upper = self.gaussian_conditional._standardized_cumulative(
                        (half - value) / scale)
                    lower = self.gaussian_conditional._standardized_cumulative(
                        (-half - value) / scale)
                    pmf = upper - lower
